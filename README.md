...
small Node build tool compiles each component into an independent JavaScript
...
`component` value created by `Component.define`.
...
add that directory to the application's `.gitignore`. It writes one classic
browser script per component to each configured `output`. Elm compilation
...
```html
<script src="dist/disclosure.js" defer></script>
...
- Each output is self-contained and includes its own Elm runtime. Multiple
  components on one page therefore duplicate runtime code. The builder does
  not bundle or minify the output.
